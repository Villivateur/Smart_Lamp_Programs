...
bool Pyro_enabled=false;//热释电是否启用
...
void Check_Pyro()
{
  if (digitalRead(Pyro)==HIGH)
  { 
    human=1;
    if (LED_warm_power+LED_cold_power==0)
    {
      LED_warm_power=255;
      LED_cold_power=255;
    }
  ...
  if (human == 1) update1(DEVICEID,"3434",human);
}
...
void Check_Battery()
{
  ...
  Battery = sensorValue * (5.0 / 1023.0)*(3.53/4.46); //此处进行了多次经验化的校准
...
void SendTempHumPyro()   // 温湿度模块 
{
   int chk = DHT11.read(DHT11PIN);      
   int Temperature_float=(float)DHT11.temperature-2.0;      //温度,经验化校准
